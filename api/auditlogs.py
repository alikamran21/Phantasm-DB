# api/auditlogs.py — Vercel Python Serverless Function
# api/health.py
...
async def _run(token, skip, limit):
    async with _SessionLocal() as db:
        ...
        r    = await db.execute(select(ForensicLedger).order_by(ForensicLedger.timestamp.desc()).offset(skip).limit(limit))
        logs = r.scalars().all()
        return 200, {"total": len(logs), "logs": [
            {"id": l.log_id, "timestamp": l.timestamp.isoformat() if l.timestamp else None,
             "user_id": l.user_id, "action": l.action, "endpoint": l.endpoint,
             "http_method": l.http_method, "is_malicious": l.is_malicious,
             "is_honeypot_action": l.was_deceived, "detection_reason": l.detection_reason,
             "response_status": l.response_status} for l in logs]}

def handler(request, context=None):
    if request.method == "OPTIONS": return preflight()
    if request.method != "GET":     return err("Method not allowed.", 405)
    query = getattr(request, "query", {}) or {}
    skip  = int(query.get("skip", 0)); limit = min(int(query.get("limit", 100)), 500)
