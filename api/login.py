# api/login.py — Vercel Python Serverless Function
# api/health.py
...
async def _run(body, ip, ua):
    if not check_rate_limit(ip): return 429, {"detail": "Too many requests."}
    email    = str(body.get("email", "")).strip().lower()
    password = str(body.get("password", ""))
    if not email or not password: return 400, {"detail": "Email and password required."}
    ...
    async with _SessionLocal() as db:
        ...
        if attack:
            ...
            if user: user.is_flagged_as_attacker = True; await db.commit()
            await write_log(db, ip, f"ATTACK:{category}", "/api/login", user_agent=ua,
                           user_id=user.identity_id if user else None, payload=email[:80], is_malicious=True)
        ...
        user.failed_login_count = 0; await db.commit()
        otp_code = generate_otp()
        db.add(OTPRequest(user_id=user.identity_id, otp_code=otp_code,
            expires_at=datetime.now(timezone.utc) + timedelta(minutes=OTP_EXPIRE), created_ip=ip))
        await db.commit()
        try: send_otp_email(user.email, otp_code, user.username or "")
        except Exception as e: return 503, {"detail": f"Email failed: {e}"}
