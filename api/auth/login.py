...
from _security import check_rate_limit, increment_failed_login, scan_for_attacks, write_audit_log
...
async def _handle(body: dict, ip: str, user_agent: str) -> tuple[int, dict]:
    ...
    async with _SessionLocal() as db:
        result = await db.execute(
            select(ProductionUser).where(ProductionUser.email == email)
        )
        ...
        return 200, {
            "detail":      "OTP dispatched. Check your email.",
