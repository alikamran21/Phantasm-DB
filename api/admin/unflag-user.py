...
"""POST /api/admin/unflag-user — clear the attacker flag on a user. Admin only."""
import asyncio
import json
import os
import sys
...
async def _handle(token: str, body: dict, ip: str, user_agent: str) -> tuple[int, dict]:
    ...
    if not target_id:
        return 400, {"detail": "user_id is required in request body."}
    ...
    async with _SessionLocal() as db:
        ...
        result = await db.execute(select(User).where(User.id == int(target_id)))
        ...
        await write_audit_log(
            db, ip, f"ADMIN_UNFLAG_USER:{target_id}",
            "/api/admin/unflag-user",
            method="POST", user_agent=user_agent,
            user_id=user.id,
        )

        return 200, {"detail": f"User {target_id} ({target.email}) has been unflagged."}
