# api/patients.py — Vercel Python Serverless Function
# api/health.py
...
async def _run(token, ip, ua):
    async with _SessionLocal() as db:
        ...
        result = await db.execute(
            ...
            .where(ProductionUser.is_active == True))
        rows = result.all()
        patients = []
        for p, pu in rows:
            meds = []
            ...
            patients.append({"id": p.identity_id, "mrn": p.mrn,
                "full_name": f"{p.first_name or ''} {p.last_name or ''}".strip() or pu.username,
                "diagnosis": p.diagnosis, "medications": meds, "notes": p.clinical_notes,
