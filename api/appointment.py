# api/appointment.py — Vercel Python Serverless Function
# api/health.py
...
async def _run(token, body, ip, ua):
    ...
    if not datetime_str: return 400, {"detail": "datetime_str required."}
    async with _SessionLocal() as db:
        ...
        await honeypot_gate(db, user, ip, ua, "PATIENT_SCHEDULE_APPT", "/api/appointment",
                            payload=json.dumps({"datetime": datetime_str}))
