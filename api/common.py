...
import json, logging, os, random, re, ssl, string, time
...
SessionLocal = async_sessionmaker(bind=_engine, class_=AsyncSession,
...
SHADOW_PATIENTS = [
    {"mrn":"PT-SHADOW-001","full_name":"Eleanor Voss",
     "diagnosis":"Generalised Anxiety Disorder (F41.1)", # Fixed key
     "active_treatment":"Sertraline 50mg","status":"Stable",
     "doctor":"Dr. Fatima Rehman","next_appointment":"Aug 12, 2025 @ 10:00 AM", "sessions": 14}, # Fixed keys & added sessions
    {"mrn":"PT-SHADOW-002","full_name":"Marcus Delray",
     "diagnosis":"Major Depressive Disorder (F32.1)",
     "active_treatment":"Fluoxetine 20mg","status":"Improving",
     "doctor":"Dr. Ali Kamran","next_appointment":"Aug 15, 2025 @ 02:30 PM", "sessions": 8},
    {"mrn":"PT-SHADOW-003","full_name":"Priya Nair",
     "diagnosis":"Bipolar II Disorder (F31.81)",
     "active_treatment":"Lamotrigine 100mg","status":"Stable",
     "doctor":"Dr. Sarah Jenkins","next_appointment":"Aug 20, 2025 @ 09:00 AM", "sessions": 22},
]
...
class OTPRequest(Base):
    ...
    def is_valid(self):
        ...
        # Ensure expires_at is timezone-aware (asyncpg may return naive UTC datetimes)
...
async def flag_threat(db, ip: str, reason: str, level: str = "high"):
    """Insert into monitor.threat_actors and return threat_id."""
    try:
...
async def log_login(db, email: str, ip: str, success: bool):
    """Write to login_activity in its own session so failures never affect the caller."""
    try:
...
async def get_user(token, db) -> tuple:
    """Decode JWT → return (User, None) or (None, error_str)."""
    if not token:
...
async def is_flagged(db, user_id: str) -> bool:
    """Check monitor.threat_actors for this IP/user — simplified: check JWT honeypot claim."""
    # The JWT already carries the honeypot flag set at login time.
    # For live re-check from DB we look up the user's email in login_activity
    # and check if they appear in threat_actors. We keep it simple here.
    return False  # trust the JWT; flag is set at verifyotp time

async def honeypot_gate(db, user, ip: str, ua: str, action: str, table: str) -> bool:
    """
    Routing logic per architecture spec:
      if user JWT has honeypot=True  → use shadow_vault
      else                           → use public schema
    Also logs to forensic_ledger.
    """
    # Decode token again to read the honeypot flag (user object doesn't carry it)
    # We pass it through a thread-local but simplest is to store on user object via __dict__
...
def mark_honeypot(user, flag: bool):
    """Tag the in-memory user object after JWT decode so honeypot_gate can read it."""
