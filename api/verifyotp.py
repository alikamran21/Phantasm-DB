# api/verifyotp.py — Vercel Python Serverless Function
# api/health.py
...
async def _run(body, ip, ua):
    ...
    if not internal_id or not otp: return 400, {"detail": "Verification code required."}
    async with _SessionLocal() as db:
        r    = await db.execute(select(ProductionUser).where(ProductionUser.identity_id == int(internal_id)))
        user = r.scalar_one_or_none()
        if not user: return 401, {"detail": "Invalid code."}
        if scan_for_attacks(otp):
            user.is_flagged_as_attacker = True; await db.commit()
        r2 = await db.execute(
            select(OTPRequest).where(OTPRequest.user_id == user.identity_id, OTPRequest.is_used == False)
        ...
        if not otp_rec or not otp_rec.is_valid() or otp_rec.otp_code != otp:
            await write_log(db, ip, "OTP_FAIL", "/api/verifyotp", user_agent=ua, user_id=user.identity_id, status=401)
            return 401, {"detail": "Invalid or expired code. Please try again."}
        otp_rec.is_used = True; await db.commit()
