...
async def _run(body, ip, ua):
    ...
    password  = str(body.get("password", "")).strip() # NEW: Capture the password
    ...
    async with SessionLocal() as db:
        ...
        if role_hint == "admin":
            if raw_id in ("admin_root", "admin", "root", "admin@serenity.care"):
                ...
                # NEW: Authenticate the hashed password
                if user and user.password_hash:
                    # Generate SHA-256 hash of provided password to compare against DB
