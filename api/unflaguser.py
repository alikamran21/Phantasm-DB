# api/unflaguser.py — Vercel Python Serverless Function
# api/health.py
...
async def _run(token, body, ip, ua):
    ...
    if not target_id: return 400, {"detail": "user_id required."}
    async with _SessionLocal() as db:
        ...
        target.is_flagged_as_attacker = False; target.failed_login_count = 0
        await db.commit()
        await write_log(db, ip, f"ADMIN_UNFLAG:{target_id}", "/api/unflaguser", user_agent=ua, user_id=user.identity_id)
